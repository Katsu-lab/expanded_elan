	...
	*/
    ...
    public static long toMilliSeconds(String timeString) {
        try {
            ...
            // 130.628 * 1000.0 = 130627.99999  !
            // long l = (long) 130627.99999 = 130627
            if (negative) {
                return Math.round( -(1000.0d * ((hours * 3600.0) + (minutes * 60.0) +
                        seconds)));
            } else {
	            return Math.round( (1000.0d * ((hours * 3600.0) + (minutes * 60.0) +
	            seconds)));
    ...
    public static long toMilliSeconds(String timeString, TIME_FORMAT format) {
    	...
    	try {
    		...
    		case MS:
        		...
        		break;
    		case SSMS:
    			t = Math.round((SEC_MS * Double.parseDouble(timeString)));
    			
    			break;
    		case HHMMSSMS:
    			String[] hparts = timeString.split(":");
    			// assume last part is seconds and possibly milliseconds
    			t = Math.round((SEC_MS * Double.parseDouble(hparts[hparts.length - 1])));
