 ...
 * duration of each  new annotation is the duration of the overlap.
 *
 * @author Han Sloetjes
 ...
 */
public class AnnotationFromOverlaps implements ProcessReporter {
    ...
    private boolean createAnnotations(TranscriptionImpl transcription) {    	
    	...
    	if(parentTierName != null){
            ...
            if (parentTier == null ) { 
            	LOG.severe("The parent tier selected is not found in this transcription" + transcription.getName());            	
    	...
    	if( processMode == MULTI_FILE_MODE ){
    		...
    		if(typeToUse == null){
               ...
               report("Selected Linguistic type is created in this transcription.");
    			...
    			if(!validType){
    				LOG.severe("Linguistic type for the tier to be created in this transcription is not compatible with the selected type: " + transcription.getName());
        ...
        for( int i=0; i<annValRecordList.size(); i++ ){
        	...
        	if (endTime <= beginTime) {
        		LOG.warning("Cannot create annotation, begin time >= end time: " + beginTime + " - " + endTime);
        		continue;
        	}
        	...
        	if (aa != null) {
                ...
                LOG.warning("Annotation could not be created " +
                		beginTime + "-" + endTime);
            }
        }
        ...
        if(processMode == SINGLE_FILE_MODE){
        	LOG.info("Number of annotations created: " + numAnnCreated );   
        }        
     ...
     * @param transcription the transcription containing the tiers (not used in this implementation)
     ...
     * @param overlapsCriteria the criteria used to check if constraints are obeyed (overlap, equal value, unequal value, or user specified constraints).
     * @param constraint the value that newValue should have to obey the constraint.
     * @return {@code true} if the values and constraints match 
     */
     ...
     *
     * @return the process report, or {@code null}
     */
    @Override
