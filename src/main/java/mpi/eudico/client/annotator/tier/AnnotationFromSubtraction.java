...
public class AnnotationFromSubtraction extends AnnotationFromOverlaps{
     ...
     * 
     * @return a list of records of subtraction results
     */
    @Override
    ...
    private List<AnnotationValuesRecord> getSubtractsList(List<TierImpl> tierList, TranscriptionImpl transcription) {
    	...
    	if(currentTier == null){
    		LOG.severe("The reference tier from which the other tiers to be subtracted is not found in this transcription" + transcription.getName());            	
    ...
    /**
     * Given a list of tiers, it computes the subtraction (based on exclusiveOR)
     * of annotations on all selected tiers.
     * 
     * @param tierList the list of tiers where the subtraction is computed for    
     * @return a list of records of subtraction results
     */
