...
/**
 * An Analyzer Host Context which also provides access to lexicons.
 */
public class TextAnalyzerLexiconHostContext implements
	...
	public void ensureLexiconFolder() {
		...
		if (prefLexFolder != null) {
			lexiconFolder = new File(prefLexFolder);
			if (!lexiconFolder.exists() || !lexiconFolder.isDirectory() || 
					(!lexiconFolder.canRead() && !lexiconFolder.canWrite())) {
				// allow a reset to default
				lexiconFolder = null;
			}
		}
