...
public class IGTViewer extends JComponent
	 ...
	 * @param lexHostContext the host providing access to a lexicon 
	 ...
	 * 
	 * @param transcription the document to store preferences for
	 */
	...
	private void storeGUIPreferences(Transcription transcription) {
		Preferences.set("IGTViewerRenderInfo.headerWidth",
				Integer.valueOf(viewerRenderInfo.headerWidth), transcription);
	}
	 ...
	 * but it does not hold on to it.
	 * 
	 * @param transcription the document to load preferences for
	 */
	 ...
	 * 
	 * @param tierColorMap mapping from tier name to color
	 */
	 ...
	 * 
	 * @param fontMap the tier name to font map
	 */
	 ...
	 * @param notifyForChanges whether or not listeners should be notified of changes in view dimension
	 */
	 ...
	 * 
	 * @param always if {@code true} the recalculation is performed regardless
	 * of changes in the width, otherwise nothing is done if the width didn't
	 * change
	 */
...
// ################# IGTViewerModelListener implementation ################################
	 ...
	 * @param row the row index to make visible
	 */
