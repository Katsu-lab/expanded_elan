...
public class AnnotationTable extends JTable {
	 ...
	 * Constructor.
	 * 
	 * @param dataModel the table model for this table
	...
	*/
	...
	/**
	 * Sets the column width default value if true, to 0 if false
	 * 
	 * @param columnName the identifier of the column
	 * @param visible if {@code true} the column is visible, otherwise it is hidden
	 */
	 ...
	 * To be called after other columns have been set/removed or component resized
	 */
	...
	/**
	 * Returns if the column is visible (e.g. width &gt; 0).
	 * 
	 * @param columnName the identifier of the column
	 * @return {@code true} if the column is visible
	 */
	...
	/**
	 * Returns default preferred width for columns. If there is no preferred
	 * width for that column, the minimum width is returned.
	 * 
	...
	/**
	 * Returns the default max width for columns. If there no maximum width has
	 * been recorded for a column, {@code Integer#MAX_VALUE} is returned. 
	 * @param columnName the column name
	...
	/**
	  * Method from ElanLocaleListener, updates the header labels.
	  */
	...
	/**
	 * Updates the font size and adjusts the row height.
	 * 
	 ...
	 * font size Font objects are created.
	 * 
	 ...
	 * from the {@code Annotation->Tier->Linguistic Type->CV->Entry(by Id)->Color}.
	 * This can be cached as it is looked up, in particular the {@code TierName->CV}.
	 * 
	 ...
	 * 
	 * @param deselectCommits the new deselectCommits flag
	 */
