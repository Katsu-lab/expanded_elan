...
/**
 * A specialized class loader for recognizer extensions.
 */
public class RecognizerLoader extends URLClassLoader {
	 ...
	 * @param nativeLibs the absolute path to native libraries, can be {@code null}
	 */
	 ...
	 * On Mac: names start with lib and have extension .jnilib or dylib<br>
		...
		if (libMap != null) {
			...
			while (mapIt.hasNext()) {
				...
				try {
					...
					LOG.warning("Cannot load library: " + url.getFile() + ". " + se.getMessage());
				} catch (UnsatisfiedLinkError ue) {
					LOG.warning("Cannot load library: " + url.getFile() + ". " + ue.getMessage());
				}
			}
		}
	}
