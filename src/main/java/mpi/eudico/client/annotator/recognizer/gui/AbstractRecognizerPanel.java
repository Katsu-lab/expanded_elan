...
/**
 * The base class for a panel providing the UI for configuration of and
 * interaction with recognizers.
 */
...
RecognizerHost, ElanLocaleListener, ItemListener, ACMEditListener {
	 ...
	 *  
	 *  @param viewerManager the {@code ViewerManager2} providing access to media files etc.
	 */
	 ...
	 * 
	 * @param event the update tier event
	 */
...
//			}
		...
		try {
			...
			if (currentRecognizer instanceof SharedRecognizer) {
				try {
					...
					LOG.warning("Cannot change the file permissions: " + ex.getMessage());
				}
			}
	...
	private void createOutputFile(String path) {
		...
		try {
			...
			LOG.warning("Cannot create the file: " + ioe.getMessage());
		} catch (SecurityException se) {
			LOG.warning("Cannot create the file: " + se.getMessage());
		}
	}
	...
	private void changeFileAccess(File f) {
		...
		*/
			...
			try {
				...
				if (exit != 0) {
					LOG.warning("Could not set the file access attributes via using native tool, error: " + exit);
				}
			} catch (IOException ioe) {
				LOG.warning("Cannot set the file access attributes via native tool");
			}
		/*	
		} catch (Exception ex) {
			// any other exception
			LOG.warning("Cannot set the file access attributes");
		}
		*/
	}
...
//		System.err.printf("message: '%s'\n", message);
	 ...
	 * 
	 * @return a {@code TierSelectionPanel} or {@code null}
	 */
	@Override
	 ...
	 * 
	 * @param paramComp the parameter component to attach
	 */
...
//			}
	...
	/**
	 * Checks if help is available for the current recognizer.
	 * 
	 * @return {@code true} if the recognizer provides help information
	 */
	 ...
	 * @param value a list of selections or the name of a tier
	 ...
	 * 
	 * @param toAppend the text to append to the report
	 */
	...
	/**
	 * Calls the "getReport()" method on the current Recognizer, using reflection.
	 ...
	 * 
	 * @param audioFilePaths the audio file paths to set
	 */
	 ...
	 * 
	 * @param videoFilePaths the video file paths to set
	 */
	 ...
	 * 
	 * @param otherFilePaths the paths to other types of files to set
	 */
