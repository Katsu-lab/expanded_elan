...
public class NumParamPanel extends AbstractParamPanel implements ChangeListener, 
	 ...
	 * @param precision one of the constants {@code NumParam#INT} and {@code NumParam##FLOAT}
	 */
	...
	/**
	 * Constructor taking a {@code NumParam} as an argument.
	 * 
	 * @param param the numerical parameter object
	 */
	 ...
	 * 
	 * @return the value of the slider, if it exists
	 */
	@Override
	protected Object getParamValue() {
		...
		if (slider != null) {
			if(type.equals(NumParam.INT)){	
				...
				return Float.valueOf(slider.getValue() / (float) scale);
			}
		}
