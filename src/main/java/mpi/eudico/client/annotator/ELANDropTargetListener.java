			...
			for (DataFlavor df : dtde.getCurrentDataFlavors()) {
				if (df.isFlavorJavaFileListType()) {
					...
					try {
						...
						// error message?
						LOG.warning("Unable to handle the dropped data: " + ufe.getMessage());
						dtde.dropComplete(false);
					} catch (IOException ioe) {
						LOG.warning("Unable to handle the dropped data: " + ioe.getMessage());
						dtde.dropComplete(false);
					} catch (Throwable t) {
						LOG.warning("Unable to handle the dropped data: " + t.getMessage());
						dtde.dropComplete(false);
					}
				}
			}
