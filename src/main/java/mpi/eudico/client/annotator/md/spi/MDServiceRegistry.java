...
public class MDServiceRegistry {
    ...
    public MDServiceProvider getProviderForMDFile(String filePath) {
        ...
        if (!f.exists()) {
        	LOG.warning("The metadata file does not exist: " + filePath);
        	return null;
        }
        if (!f.canRead()) {
        	LOG.warning("The metadata file cannot be read: " + filePath);
        	return null;
        }
        if (f.length() == 0) {
        	LOG.warning("The metadata file has zero length: " + filePath);
        	return null;
        }
