...
public class MergeTierGroupCommand implements UndoableCommand {
	...
	public void execute(Object receiver, Object[] arguments) {
        ...
        if (copySuffix == null) {
            ...
            LOG.warning("Name of destination tier is null, changed to MergedGroup");
        }
        ...
        if (destTier != null) {
            ...
            LOG.warning("Tier " + (sourceTierName1 + copySuffix) +
        ...
        if ((transcription != null) && (destTierName != null)) {
            ...
            for (int i = 0; i < tierRecords.size(); i++) {
            	...
            	if (transcription.getTierWithId(tr.getName()) == null) {
            		if (tr.getParentTier() != null) {
            			...
            			if (pt == null) {
            				// error situation
            				LOG.severe("Cannot recreate tier: " + tr.getName() + 
            		...
            		LOG.warning("Tier already exists!");
            	}
            }

            if (transcription.getTierWithId(destTierName) == null) {
                //transcription.addTier(destTier);
            	LOG.severe("Could not recreate tier group, exiting...");
                setWaitCursor(false);
            ...
            if ((annRecords != null) && (annRecords.size() > 0)) {
                ...
                for (int i = 0; i < annRecords.size(); i++) {
                    ...
                    if (ann == null) {
                    	LOG.warning("Could not recreate annotation: " + 
    ...
    private class MergeThread extends Thread {
		...
		public void run() {
            ...
            for (int i = 0; i < numAnns2; i++) {
            	...
            	if (ann2 != null) {
            		...
            		if (numSkippedAnnos == 0) {
            			LOG.warning("Could not recreate annotation on new tier group.");
            		}
            		numSkippedAnnos++;
            	}
            ...
            if (numSkippedAnnos > 1) {
            	LOG.warning(numSkippedAnnos + " annotations could not be recreated.");
            }
            // restore the time propagation mode
            transcription.setTimeChangePropagationMode(curPropMode);
