 ...
 * @version 1.0
 */
public class CSVServiceProvider implements TSServiceProvider {
    private final String[] suffices = new String[] { "txt", "csv" };

    // a source file -> reader map; keep one reader per source
    private HashMap<String, CSVReader> sourceToReaderMap;
    ...
    public CSVServiceProvider() {
        super();
        sourceToReaderMap = new HashMap<String, CSVReader>();
    }
    ...
    /**
     * Returns {@code true}. This provider supports configuration of tracks by
     * supplying a configuration panel.
     *
     * @return {@code true} tracks can and must be configured by the user through a
     *         configuration panel
     */
    @Override
	...
	public TSConfigPanel getConfigPanel(TSSourceConfiguration config) {
        ...
        if (sourceToReaderMap.get(config.getSource()) != null) {
            configPanel.setReader(sourceToReaderMap.get(
                    config.getSource()));
        } else {
