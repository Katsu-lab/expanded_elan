...
public class DefaultTimeScaleBasedViewer extends TimeScaleBasedViewer
     ...
     * There are two special cases taken into account:
     * 
     * <ul>
     * <li>
     ...
     *
     * @param mediaTime the current media time which should be visible
     */
    protected void recalculateInterval(final long mediaTime) {
		...
		if (newBeginTime == intervalBeginTime && newEndTime == intervalEndTime) {
			// nothing changed
			return;
		}
     ...
     * 
     * @param visible the visibility flag for the time ruler
     */
    ...
    /**
     * Checks the selection begin and end time and calculates pixel positions.
     *
	...
	public void mouseWheelMoved(MouseWheelEvent e) {
    	...
    	if (e.isControlDown()) {
	        ...
	        int timeDiff = (int) (intervalWidth * msPerPixel);
	        if (e.getWheelRotation() < 0) {
	        	timeDiff = -timeDiff;
	        }
	        long newTime = intervalBeginTime + timeDiff > 0 ? intervalBeginTime + timeDiff : 0;
