...
(JNIEnv *env, jobject callerObject, jstring mediaURL) {
    ...
    if ([AVFLog isLoggable:AVFLogInfo]) {
        mjlogfWE(env, "N_AVFNativePlayer.initPlayer: media URL: %s", [urlString UTF8String]);
    }
    ...
    if ([AVFLog isLoggable:AVFLogFine]) {
        mjlogfWE(env, "N_AVFNativePlayer.initPlayer: media URL has file protocol: %i, is absolute: %i", fileProt, absFile);
    }
    if (fileProt || absFile) {
    ...
    if (avfPlayer != NULL) {
        if ([avfPlayer lastError] != NULL) {//??
            if ([AVFLog isLoggable:AVFLogWarning]) {
                mjlogfWE(env, "N_AVFNativePlayer.initPlayer: error while creating player: %s", [[avfPlayer lastError] description] != NULL ? [[[avfPlayer lastError] description] UTF8String] : "nil");
    ...
    if ([AVFLog isLoggable:AVFLogInfo]) {
        mjlogfWE(env, "N_AVFNativePlayer.initPlayer: id of new player: %ld, has audio: %s, has video: %s", nextId, ([avfPlayer hasAudio] ? "yes" : "no"), ([avfPlayer hasVideo] ? "yes" : "no"));
    }
    return nextId;
}
...
(JNIEnv *env, jobject callerObject, jlong playerId) {
    ...
    if (avfPlayer != NULL) {
        if ([AVFLog isLoggable:AVFLogInfo]) {
            mjlogfWE(env, "N_AVFNativePlayer.deletePlayer: deleting player with id: %ld", playerId);
        }
...
(JNIEnv *env, jobject callerObject, jlong playerId, jobject visualComponent) {
    PlayerConnector *playConnect = (PlayerConnector *) playerId;//ptr should be valid
    AVFNativePlayer *avfPlayer = (AVFNativePlayer *) playConnect->player;
 
    if (avfPlayer == NULL) {
        // log and return
        if ([AVFLog isLoggable:AVFLogWarning]) {
            mjlogfWE(env, "N_AVFNativePlayer.setVisualComponent: no player with id: %ld", playerId);
        }
        return;
    }
    jint compMonitor = env->MonitorEnter(visualComponent);
    if (compMonitor != JNI_OK) {
        if ([AVFLog isLoggable:AVFLogWarning]) {
            mjlogWE(env, "N_AVFNativePlayer.setVisualComponent: unable to enter the component monitor");
        }
        return;
    }
    // get the AWT
    ...
    if (result == JNI_FALSE) {
        if ([AVFLog isLoggable:AVFLogWarning]) {
            mjlogWE(env, "N_AVFNativePlayer.setVisualComponent: unable to get JAWT");
        }
        env->MonitorExit(visualComponent);// check success?
    ...
    if (ds == NULL) {
        if ([AVFLog isLoggable:AVFLogWarning]) {
            mjlogWE(env, "N_AVFNativePlayer.setVisualComponent: the JAWT_DrawingSurface for the component is null");
        }
        env->MonitorExit(visualComponent);// check success?
        return;
    }
    ...
    if ((lock & JAWT_LOCK_ERROR) != 0) {
        if ([AVFLog isLoggable:AVFLogWarning]) {
            mjlogWE(env, "N_AVFNativePlayer.setVisualComponent: unable to lock the JAWT Drawing Surface");
        }
        env->MonitorExit(visualComponent);// check success?
        return;
    }
    ...
    if (dsi != NULL) {
        if ([AVFLog isLoggable:AVFLogFine]) {
            mjlogWE(env, "N_AVFNativePlayer.setVisualComponent: got the JAWT Drawing Surface Info");
        }
        ...
        if ([AVFLog isLoggable:AVFLogFine]) {
            mjlogWE(env, "N_AVFNativePlayer.setVisualComponent: got the JAWT_SurfaceLayers");
        }
        // setLayer now works
        ...
        if ([avfPlayer firstConnectionToCanvas] == FALSE) {
            ...
            if ([AVFLog isLoggable:AVFLogFine]) {
                mjlogWE(env, "N_AVFNativePlayer.setVisualComponent: created host layer, first time connection");
            }
        } else {
            // reconnect the host layer to the surface layer
            [surfaceLayers setLayer: [avfPlayer hostLayer]];
            if ([AVFLog isLoggable:AVFLogFine]) {
                mjlogWE(env, "N_AVFNativePlayer.setVisualComponent: reconnected the host layer");
            }
        }
        ...
        if ([AVFLog isLoggable:AVFLogWarning]) {
            mjlogWE(env, "N_AVFNativePlayer.setVisualComponent: unable to get the JAWT Drawing Surface Info");
        }
    }
    ...
    if ([AVFLog isLoggable:AVFLogInfo]) {
        mjlogWE(env, "N_AVFNativePlayer.setVisualComponent: succesfully added the player layer to the Java component's surface layer");
    }
    jint compMonExit = env->MonitorExit(visualComponent);
    if (compMonExit != JNI_OK) {
        if ([AVFLog isLoggable:AVFLogWarning]) {
            mjlogWE(env, "N_AVFNativePlayer.setVisualComponent: unable to exit the component monitor");
        }
    }
}
...
(JNIEnv *env, jobject callerObject, jlong playerId, jobject visualComponent) {
    PlayerConnector *playConnect = (PlayerConnector *) playerId;
    AVFNativePlayer *avfPlayer = (AVFNativePlayer *) playConnect->player;
    
    if (avfPlayer == NULL) {
        // log and return
        if ([AVFLog isLoggable:AVFLogWarning]) {
            mjlogfWE(env, "N_AVFNativePlayer.disconnectVisualComponent: no player with id: %ld", playerId);
        }
        return;
    }
    ...
    if (result == JNI_FALSE) {
        if ([AVFLog isLoggable:AVFLogWarning]) {
            mjlogWE(env, "N_AVFNativePlayer.disconnectVisualComponent: unable to get JAWT");
        }
    ...
    if (ds == NULL) {
        if ([AVFLog isLoggable:AVFLogWarning]) {
            mjlogWE(env, "N_AVFNativePlayer.disconnectVisualComponent: the JAWT_DrawingSurface for the component is null");
        }
        return;
    }
    ...
    if ((lock & JAWT_LOCK_ERROR) != 0) {
        if ([AVFLog isLoggable:AVFLogWarning]) {
            mjlogWE(env, "N_AVFNativePlayer.disconnectVisualComponent: unable to lock the JAWT Drawing Surface");
        }
        return;
    }
    ...
    if (dsi != NULL) {
        if ([AVFLog isLoggable:AVFLogFine]) {
            mjlogWE(env, "N_AVFNativePlayer.disconnectVisualComponent: got the JAWT Drawing Surface Info");
        }
        // detach the "root layer" from the AWT Canvas surface layers
        //id <JAWT_SurfaceLayers> surfaceLayers = (__bridge id <JAWT_SurfaceLayers>)dsi->platformInfo;
        //NSLog(@"N_AVFNativePlayer_disconnectVisualComponent: got the JAWT_SurfaceLayers.");
        ...
        if ([AVFLog isLoggable:AVFLogFine]) {
            mjlogWE(env, "N_AVFNativePlayer.disconnectVisualComponent: succesfully removed the player layer from the surface layer");
        }
        
        ds->FreeDrawingSurfaceInfo(dsi);
    } else {
        if ([AVFLog isLoggable:AVFLogWarning]) {
            mjlogWE(env, "N_AVFNativePlayer.disconnectVisualComponent: unable to get the JAWT Drawing Surface Info");
        }
    }
...
(JNIEnv *env, jobject callerObject, jlong playerId, jint vx, jint vy, jint vw, jint vh) {
    ...
    if (avfPlayer != NULL) {
        if ([avfPlayer playerLayer] != NULL) {
            ...
            if ([AVFLog isLoggable:AVFLogFine]) {
                mjlogfWE(env, "N_AVFNativePlayer.setVideoBounds: setting video bounds x:%d, y:%.2f, w:%d, h:%d", vx, cy, vw, vh);
            }
