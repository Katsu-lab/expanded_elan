...
- (id) initWithURL: (NSURL *) url {
    ...
    if ([super hasVideo]) {
        ...
        if ([AVFLog isLoggable:AVFLogFine]) {
            mjlogf("AVFCustomPlayer.initWithURL: the asset has video, using pixel format 24RGB for image output.");
        }
    }
...
- (BOOL) setPixelFormat: (NSString *) formatString {
    if (outputSettings != NULL) {
        if ([formatString containsString:@"24RGB"]) {
            [outputSettings setObject:@(kCVPixelFormatType_24RGB) forKey:(id)kCVPixelBufferPixelFormatTypeKey];
            if ([AVFLog isLoggable:AVFLogFine]) {
                mjlog("AVFCustomPlayer.setPixelFormat: set pixel format to 24RGB");
            }
            return TRUE;
        }
        if ([formatString containsString:@"32ARGB"]) {
            ...
            if ([AVFLog isLoggable:AVFLogFine]) {
                mjlog("AVFCustomPlayer.setPixelFormat: set pixel format to 32ARGB");
            }
            return TRUE;
        }
        if ([formatString containsString:@"32BGRA"]) {
            ...
            if ([AVFLog isLoggable:AVFLogFine]) {
                mjlog("AVFCustomPlayer.setPixelFormat: set pixel format to 32BGRA");
            }
            return TRUE;
        }
        if ([formatString containsString:@"Gray"]) {
            ...
            if ([AVFLog isLoggable:AVFLogFine]) {
                mjlog("AVFCustomPlayer.setPixelFormat: set pixel format to Gray (8IndexedGray_WhiteIsZero)");
            }
            return TRUE;
        }
        if ([formatString containsString:@"422YpCbCr8"]) {
            ...
            if ([AVFLog isLoggable:AVFLogFine]) {
                mjlog("AVFCustomPlayer.setPixelFormat: set pixel format to 422YpCbCr8 Full Range");
            }
            return TRUE;
        }
        return FALSE;
...
- (long) getFrameSequenceFromTimeSeconds: (double)time1 toTime:(double)time2 outputBuffer:(char *)bufferAddress bufferSize:(int)bufferSize outputIndexArray:(double [])outIndices outputArraySize:(int)arraySize {
    ...
    if (readerError != nil) {
        if ([AVFLog isLoggable:AVFLogWarning]) {
            mjlogf("AVFCustomPlayer.getFrameSequenceFromTimeSeconds: AssetReader is NULL: %s", [readerError description] != NULL ? [[readerError description] UTF8String] : "");
        }
    ...
    if ([AVFLog isLoggable:AVFLogFine]) {
        mjlogf("AVFCustomPlayer.getFrameSequenceFromTimeSeconds: time range start: %.4f - duration %.4f, buffer size %d", CMTimeGetSeconds([assetReader timeRange].start), CMTimeGetSeconds([assetReader timeRange].duration), bufferSize);
    }
    ...
    if (!readyForRead) {
        if ([AVFLog isLoggable:AVFLogWarning]) {
            mjlog("AVFCustomPlayer.getFrameSequenceFromTimeSeconds: AssetReader is not ready for reading, returning");
        }
        assetReader = NULL;
        trackOutput = NULL;
    ...
    while (assetReader.status != AVAssetReaderStatusCompleted) {//assetReader.status == AVAssetReaderStatusReading
        ...
        if (sampleRef == NULL) {
            if ([AVFLog isLoggable:AVFLogFine]) {
                mjlogf("AVFCustomPlayer.getFrameSequenceFromTimeSeconds: no sample buffer, error: %s", ([assetReader error] != NULL && [[assetReader error] description] != NULL) ? [[[assetReader error] description] UTF8String] : "unknown");
            }
            ...
            if ([AVFLog isLoggable:AVFLogFine]) {
                mjlogf("AVFCustomPlayer.getFrameSequenceFromTimeSeconds: presentation time: %.4f (%.4f)", presTime, CMTimeGetSeconds(timingInfoOut.presentationTimeStamp));
            }
            
            CVImageBufferRef imgBufferRef = CMSampleBufferGetImageBuffer(sampleRef);
            if (imgBufferRef == NULL) {
                if ([AVFLog isLoggable:AVFLogWarning]) {
                    mjlog("AVFCustomPlayer.getFrameSequenceFromTimeSeconds: CVImageBufferRef is null");// what to do, return??
                }
            } else {
                if ([AVFLog isLoggable:AVFLogAll] && numBytesRead == 0 && bytesPerFrame == 0) {// log once per session
                    ...
                    mjlogf("AVFCustomPlayer.getFrameSequenceFromTimeSeconds: CVImageBufferRef Data size: %lu, Data width: %lu, Data height: %lu, Bytes per row: %lu", dataSize, CVPixelBufferGetWidth(imgBufferRef),CVPixelBufferGetHeight(imgBufferRef), CVPixelBufferGetBytesPerRow(imgBufferRef));

                    size_t padLeft, padRight;
                    size_t padTop, padBot;
                    CVPixelBufferGetExtendedPixels(imgBufferRef, &padLeft, &padRight, &padTop, &padBot);
                    mjlogf("AVFCustomPlayer.getFrameSequenceFromTimeSeconds: CVImageBufferRef Data padding: Left: %lu, Right: %lu, Top: %lu, Bottom: %lu", padLeft, padRight, padTop, padBot);
                    ...
                    //mjlogf("AVFCustomPlayer.getFrameSequenceFromTimeSeconds: CVImageBufferRef pixel format type %s", osTypeString);// how to convert?
                }
                
                CVReturn lock = CVPixelBufferLockBaseAddress(imgBufferRef, kCVPixelBufferLock_ReadOnly);
                if (lock == kCVReturnSuccess) {
                    if ([AVFLog isLoggable:AVFLogAll]){
                        mjlog("AVFCustomPlayer.getFrameSequenceFromTimeSeconds: successfully locked the pixel buffer");
                    }
                    ...
                    if (bytesPerFrame == 0) {
                        ...
                        if (videoImageWidth != self.videoWidth) {
                            if ([AVFLog isLoggable:AVFLogInfo]){
                                mjlogf("AVFCustomPlayer.getFrameSequenceFromTimeSeconds: the width of the pixel buffer is not equal to the width of the video image: %ld - %.2f", videoImageWidth, self.videoWidth);
                            }
                        }
                        if (videoImageHeight != self.videoHeight) {
                            if ([AVFLog isLoggable:AVFLogInfo]){
                                mjlogf("AVFCustomPlayer.getFrameSequenceFromTimeSeconds: the height of the pixel buffer is not equal to the height of the video image: %ld - %.2f", videoImageHeight, self.videoHeight);
                            }
                        }
                        ...
                        if ([AVFLog isLoggable:AVFLogInfo]){
                            mjlogf("AVFCustomPlayer.getFrameSequenceFromTimeSeconds: bytes per frame: %ld", bytesPerFrame);
                        }
                    }
                    ...
                    if (numBytesRead + bytesPerFrame <= bufferSize) {
                        ...
                        if (time1 < self.frameDurationSeconds && index == 0) {
                            if (presTime >= self.frameDurationSeconds) {
                                ...
                                index++;
                                if ([AVFLog isLoggable:AVFLogInfo]){
                                    mjlogf("AVFCustomPlayer.getFrameSequenceFromTimeSeconds: inserted one frame at position 0, before presentation time %.4f", presTime);
                                }
                            }
                        }
                    }
                    ...
                    if (numBytesRead + bytesPerFrame <= bufferSize) {
                        ...
                        index++;
                        if ([AVFLog isLoggable:AVFLogFine]){
                            mjlogf("AVFCustomPlayer.getFrameSequenceFromTimeSeconds: inserted frame buffer for presentation time %.4f", presTime);
                        }
                    } else {
                       if ([AVFLog isLoggable:AVFLogInfo]){
                           mjlogf("AVFCustomPlayer.getFrameSequenceFromTimeSeconds: cannot copy image, buffer too small, need: %ld, available: %ld (occupied: %ld, of buffer: %d)", bytesPerFrame, (bufferSize - numBytesRead), numBytesRead, bufferSize);
                       }
                    }
                    ...
                    if ([AVFLog isLoggable:AVFLogWarning]){
                        mjlog("AVFCustomPlayer.getFrameSequenceFromTimeSeconds: failed to lock the pixel buffer");
                    }
                }
        ...
        if (numBytesRead >= bufferSize) {
            // the buffer is full
            if ([AVFLog isLoggable:AVFLogFine]){
                mjlogf("AVFCustomPlayer.getFrameSequenceFromTimeSeconds: buffer is filled at %.4f seconds", outIndices[index - 1]);
            }
            break;
        }
        ...
        if (index > 0 && outIndices[index - 1] + self.frameDurationSeconds > time2) {
            // next read will fail
            if ([AVFLog isLoggable:AVFLogFine]){
                mjlogf("AVFCustomPlayer.getFrameSequenceFromTimeSeconds: breaking read loop at %.4f seconds", outIndices[index - 1]);
            }
        ...
        if (assetReader.status == AVAssetReaderStatusCompleted) {
            if ([AVFLog isLoggable:AVFLogFine]){
                mjlog("AVFCustomPlayer.getFrameSequenceFromTimeSeconds: Reader completed");
            }
            [assetReader cancelReading];
            break;
        } else if (assetReader.status == AVAssetReaderStatusFailed) {
            if ([AVFLog isLoggable:AVFLogFine]){
                mjlog("AVFCustomPlayer.getFrameSequenceFromTimeSeconds: Reader failed");
            }
            [assetReader cancelReading];
            break;
        }
    ...
    if (arraySize >= 3) {
        if ([AVFLog isLoggable:AVFLogFine]){
            mjlogf("AVFCustomPlayer.getFrameSequenceFromTimeSeconds: first three time stamps: %.4f, %.4f, %.4f", outIndices[0], outIndices[1], outIndices[2]);
        }
        if (outIndices[0] != -1 && outIndices[1] != -1 && outIndices[2] != -1) {
            ...
            if ([AVFLog isLoggable:AVFLogFine]){
                 mjlogf("AVFCustomPlayer,getFrameSequenceFromTimeSeconds: first three time stamps after correction: %.4f, %.4f, %.4f", outIndices[0], outIndices[1], outIndices[2]);
            }
        }
    }
    
    if ([AVFLog isLoggable:AVFLogFine]){
        mjlogf("AVFCustomPlayer.getFrameSequenceFromTimeSeconds: total number of bytes read: %ld (from %.4f to %.4f)", numBytesRead, time1, time2);
    }
...
- (void) detectFrameSizeInBytes {
    ...
    if (self.mediaAsset != nil && self.hasVideo) {
        ...
        if ([AVFLog isLoggable:AVFLogFine]){
            mjlogf("AVFCustomPlayer.detectFrameSizeInBytes: \n\
